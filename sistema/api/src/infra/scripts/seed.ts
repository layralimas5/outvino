/**
 * Popula o sistema com catálogo de exemplo pra loja não subir vazia.
 *
 * Os vinhos são fictícios: vinícola, safra e nota de degustação foram
 * escritos como amostra. Quando o catálogo real do cliente chegar, apague os
 * dados (`sistema/api/dados/*.json`) e cadastre pelo painel.
 *
 * Uso: `npm run seed` (recusa se já houver produto) · `npm run seed -- --forcar`
 */
import { mkdir } from 'node:fs/promises';
import { env } from '../../config/env.js';
import { criarAvaliacao, moderarAvaliacao } from '../../application/avaliacoes/casosDeUso.js';
import { criarCupom } from '../../application/cupons/casosDeUso.js';
import { criarProduto } from '../../application/produtos/casosDeUso.js';
import { criarBanner, criarSecao } from '../../application/vitrine/casosDeUso.js';
import type { CriarProdutoInput } from '../../domain/produtos/Produto.js';
import { criarDeps } from '../container.js';

const PRODUTOS: CriarProdutoInput[] = [
  {
...
async function semear(): Promise<void> {
  ...
  if (existentes.length > 0 && !forcar) {
    console.log(
      `Já existem ${existentes.length} produtos em ${env.dataDir}. Nada foi alterado.\n` +
        'Use "npm run seed -- --forcar" se quiser cadastrar os exemplos mesmo assim.',
    );
    return;
  }

  for (const entrada of PRODUTOS) {
    const produto = await criarProduto(deps, entrada, 'seed');
  ...
  console.log('cupom: PRIMEIRACOMPRA');

  // Já publicadas: sem isso o seed criaria uma fila de moderação em vez de
  // uma loja com avaliação pronta pra ver.
  for (const entrada of AVALIACOES) {
    const { id } = await criarAvaliacao(deps, entrada);
    await moderarAvaliacao(deps, id, { status: 'publicada', resposta: entrada.resposta }, 'seed');
  }
